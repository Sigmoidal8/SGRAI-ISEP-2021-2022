...
export default class Ball {
    ...
    update(deltaT) {
        ...
        let incrementX = coveredDistance * Math.cos(this.direction);
        let incrementY = coveredDistance * Math.sin(this.direction);
        const centerIncrement = new THREE.Vector3(incrementX,incrementY,0.0);
        ...
        /* To-do #12 - Check if the ball hit player 2 racket
            ...
            if ((this.center.x + this.radius) > (this.player2.center.x-this.player2.halfSize.x) &&
                ...
                (this.center.y >= (this.player2.center.y - this.player2.halfSize.y))) { // The ball hit player 2 racket
                this.direction = Math.PI-this.direction;  // The ball rebounds
            }
        } 
        ...
        if (centerIncrement.y < 0 && (this.center.y < (-this.table.halfSize.y + this.radius)) || // The ball is moving down and hit the bottom line
            ...
            this.direction = 2*Math.PI - this.direction; // The ball rebounds
            if (centerIncrement.x < 0.0) {
                this.direction += 2.0 * Math.PI; // This is to ensure that the ball direction stays within the interval 0.0 <= direction < 2.0 * π (pi)
            }
        }
