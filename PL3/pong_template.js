...
export default class Pong {
    ...
    update() {
        if (this.gameRunning && !this.gamePaused) {
            ...
            if (this.ball.center.x < -this.table.halfSize.x-this.ball.radius) { // Player 1 scored
                this.player1.score++; // Increment player 1 score
                this.ball.initialize();
                this.displayScore();
            }
            else if (this.ball.center.x > this.table.halfSize.x+this.ball.radius) { // Player 2 scored
