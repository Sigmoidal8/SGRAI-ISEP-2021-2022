...
export default class Watch {
    constructor(cityName, center = new THREE.Vector3(0.0, 0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        for(let i=0; i<60; i++){
            ...
            let x = x0 + radius2 * Math.cos(startingAngle);
            let y = y0 + radius2 * Math.sin(startingAngle);
            ...
            if(i==0 || i%5 == 0){
                x1=x0 + radius0 * Math.cos(startingAngle);
                y1=y0 + radius0 * Math.sin(startingAngle);
            }else{
                x1=x0 + radius1 * Math.cos(startingAngle);
                y1=y0 + radius1 * Math.sin(startingAngle);
            }

            points.push(new THREE.Vector3(x,y,z0));
            points.push(new THREE.Vector3(x1,y1,z0));
        ...
        */
        ...
        const vertices = new Float32Array([-0.1* radius, 0.0, 0.0, 0.0, 0.025*radius, 0.0, 0.0, -0.025 * radius, 0.0, 
            -0.1 * radius, 0.0, 0.0, 0.0, 0.025 * radius, 0.0, 0.0, -0.025 * radius, 0.0, 
            0.0,0.025 * radius,0.0, 0.5 * radius,0.0,0.0, 0.0,-0.025 * radius, 0.0,
            0.0,0.025 * radius,0.0, 0.5 * radius,0.0,0.0, 0.0,-0.025 * radius, 0.0    
        ]);
        ...
        */
        ...
        const verticesS = new Float32Array([-0.14* radius, 0.0, 0.0, 0.0, 0.035*radius, 0.0, 0.0, -0.035 * radius, 0.0, 
            -0.14 * radius, 0.0, 0.0, 0.0, 0.035 * radius, 0.0, 0.0, -0.035 * radius, 0.0, 
            0.0,0.035 * radius,0.0, 0.7 * radius,0.0,0.0, 0.0,-0.035 * radius, 0.0,
            0.0,0.035 * radius,0.0, 0.7 * radius,0.0,0.0, 0.0,-0.035 * radius, 0.0    
        ]);

        let geometry1 = new THREE.BufferGeometry();
        geometry1.setAttribute('position', new THREE.BufferAttribute(verticesS, 3));
        material = new THREE.MeshBasicMaterial({color: handsHMColor});
        this.object.handM = new THREE.Mesh(geometry1, material);
        this.object.add(this.object.handM);
