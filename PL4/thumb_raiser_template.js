...
export default class ThumbRaiser {
    ...
    collision(position) {
        ...
        return this.maze.distanceToWestWall(position) < this.player.radius || this.maze.distanceToEastWall(position) < this.player.radius ||
            this.maze.distanceToNorthWall(position) < this.player.radius || this.maze.distanceToWestWall(position) < this.player.radius;
    }
            ...
            if (!this.animations.actionInProgress) {
                ...
                if (this.maze.foundExit(this.player.position)) {
                    ...
                    if (this.player.keyStates.backward) { // The player is moving backward
                        const newPosition = new THREE.Vector3(coveredDistance * Math.sin(direction + Math.PI), 0,
                            coveredDistance * Math.cos(direction + Math.PI)).add(this.player.position);
                        ...
                        const newPosition = new THREE.Vector3(coveredDistance * Math.sin(direction), 0,
                        ...
                        /* To-do #21 - If the player is not moving nor emoting, then trigger the idle action
                            ...
                            this.animations.fadeToAction('Idle', this.animations.activeName != 'Death' ? 0.6 : 0.2);
                        }
                    ...
                    this.player.object.position.set(this.player.x,this.player.position.y, this.player.position.z);
